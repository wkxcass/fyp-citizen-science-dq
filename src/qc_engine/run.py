...
def main() -> None:
    ...
    parser.add_argument("--input", default="data/raw/occurrences_sample.csv")
    parser.add_argument("--oracle", default="src/oracle_gen/example_oracle.py")
    parser.add_argument("--output", default="experiments/runs/v0/flagged_records.csv")
    parser.add_argument("--include-passes", action="store_true")
    args = parser.parse_args()
    print(f"Flagged {run(args.input, args.oracle, args.output, args.include_passes)} record(s). Output: {args.output}")
