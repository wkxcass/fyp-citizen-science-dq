...
This repository contains the v0.1 experimental pipeline:
...
- Dataset representation: a normalized schema defined in `config/v0.1.yaml`
...
v0.1 deliberately keeps the prototype small. It currently defers intermediate constraint representations, semantic verification, user interfaces, database design, automatic correction, and automatic deletion. A flag means that a record deserves review; it is not proof that the record is erroneous.
...
The repository loads this file automatically when a command loads `config/v0.1.yaml`. `.env` is ignored by Git and must never be committed.

`config/v0.1.yaml` is the canonical v0.1 experiment configuration. It defines the species, source, iNaturalist endpoint, normalized schema, model, prompt path, input/output paths, and generated-oracle path. Command-line arguments are available as explicit per-run overrides.
...
```text
experiments/runs/v0.1/flagged_records.csv
```
...
```bash
python -m src.qc_engine.inaturalist --config config/v0.1.yaml
```
...
```bash
python -m src.oracle_gen.generate --config config/v0.1.yaml --prompt-only
```
...
```bash
python -m src.oracle_gen.generate --config config/v0.1.yaml
```
...
The generated file is written to `experiments/runs/v0.1/generated_oracle.py`. Inspect it before execution: it should define `check_record(record)` and should not contain Markdown code-fence markers.
...
```bash
python -m src.qc_engine.run --config config/v0.1.yaml
```
...
The QC runner dynamically loads the configured oracle, applies `check_record(record)` to each normalized occurrence, and writes flagged records to `experiments/runs/v0.1/flagged_records.csv`.
...
- Review generated Python before running it. v0.1 generates executable code, so the generated artifact is an experimental input rather than trusted production code.
...
## ℹ️ AI Tool Disclosure

AI-assisted tools, including GPT-5.6 and Claude Sonnet 5, were utilized during the preparation and development of this repository. Specifically, these models assisted with:
- Documentation Synthesis: Generating and structuring documentation content directly from manually maintained FYP notes and logs.
- Code Implementation: Translating high-level architecture, design frameworks and development plans presented by the author into functional code.
