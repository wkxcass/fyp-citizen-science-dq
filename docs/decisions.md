...
- **D011 — Schema/runtime type-contract mismatch**
  - Status: Tentative
  - Decision: TBD — either (a) have the QC engine coerce record field values to their configured schema `type` (and normalize CSV empty-string to `None`) before calling `check_record`, or (b) make the oracle-generation prompt explicitly state the actual runtime representation (raw CSV strings; missing = `""`, not `None`), so generated oracles cast defensively and correctly.
  ...
  - Priority note: silently defeats the only currently testable (spatial) constraint category regardless of prompt/model quality — recommend resolving before D013's harness is built.
...
- **D013 — Mutation-testing feedback loop**
  ...
  - Dependency: Blocked on D011 — a broken spatial-check plumbing bug would make detection-rate numbers uninterpretable until fixed.
